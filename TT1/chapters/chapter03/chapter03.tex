...
\begin{figure}[h]
	...
	\begin{tikzpicture}[scale=1.0]
		...
		\draw[domain=0:6,smooth,variable=\x,blue,thick] plot ({\x},{1.8*sin(180*\x/6)});
		
		% Etiqueta de la cuerda
		\node[above,blue] at (3,1.9) {$u(x,0) = f(x)$};
	...
	\end{tikzpicture}
	\caption[Ilustración del problema de la cuerda vibrante.] {\textit{Fuente: Elaboración propia} }
...
\begin{figure}[h]
	...
	\caption[Ilustración de 3 armónicos y sus nodos de la cuerda al vibrar.] {\textit{Fuente: Elaboración propia}} 
...
\begin{figure}[h]
	...
	\end{tikzpicture}
 	\caption[Ilustración del problema de transferencia de calor en una barra de longitud $\pi$.  ]{\textit{ Fuente: Elaboración propia} }
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=1]
		\begin{axis}[
			...
			\addplot[blue, ultra thick] {2 * ((-1)^(1-1)/1 * sin(deg(1*x)) + (-1)^(2-1)/2 * sin(deg(2*x)) + (-1)^(3-1)/3 * sin(deg(3*x)))};    
	...
	\end{tikzpicture}
		...
		\caption[Gráfica de una función periódica $f(t)$ con periodo $T$.]  {\textit{ Fuente: Elaboración propia} }
...
\begin{figure}
	...
	\begin{tikzpicture}[scale=1]
		...
		\end{axis}
	\end{tikzpicture}
	\caption[Gráfica del área bajo la curva de $f(t)$ entre los intervalos desde $a$ hasta $a + T$ y desde $b$ hasta $b + T$. ]{\textit{ Fuente: Elaboración propia} }
	...
	\begin{minipage}{0.5\textwidth} % Reducimos el ancho al 48%
		\begin{tikzpicture}[scale=0.45]
			\begin{axis}[
				...
				\addplot[blue, ultra thick] {( sin(180*x/pi) )};
	...
	\begin{minipage}{0.5\textwidth} % Reducimos el ancho al 48%
		\begin{tikzpicture}[scale=0.45]
			\begin{axis}[
				...
				\addplot[blue, ultra thick] {( cos(180*x/pi) )};
		...
		\end{tikzpicture}
	\end{minipage}
	\caption[Gráfica de las funciones trigonométricas $sin(t)$ y $cos(t)$.  ]{\textit{ Fuente: Elaboración propia} }
...
\begin{equation}
	f = \frac{1}{T}
\end{equation}
lo cual indica el número de oscilaciones de la función por unidad de tiempo. La unidad de medida de la frecuencia es el hertz (Hz). Un hertz quiere decir que una función oscila una vez por segundo. La Figura \ref{fig:seno-diferentes-frecuencias} muestra una función senoidal con tres frecuencias diferentes.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			\draw[<->, ultra thick, blue] (0,-1.75) -- (2*pi,-1.75) node[midway, below, black] {\large $T_3$};
...
La \textit{frecuencia angular}, usualmente denotada por \( \omega \), se define como la razón de cambio del desplazamiento angular \( \theta \) durante la oscilación (o rotación) ~\cite{fourierCinvestav}:
\begin{equation}
	\frac{d\theta}{dt} = \omega = 2 \pi f = \frac{2 \pi}{T}.
\end{equation}

La frecuencia angular se mide comúnmente en radianes por segundo (\( \text{rad/s} \)). La Figura\ref{fig:frecuencia-func-circ} ejemplifica la relación de una onda senoidal en el intervalo \([0, 2\pi]\) con una frecuencia angular \( \omega \). ~\cite{fourierCinvestav}
\begin{figure}[H]
	\begin{minipage}{0.5\textwidth} % Reducimos el ancho al 48%
	...
	\begin{minipage}{0.5\textwidth} % Reducimos el ancho al 48%
		...
		\begin{tikzpicture}
			% Draw the circle
			\draw[ultra thick, blue] (0,0) circle (4cm);
	...
	\end{minipage}
	\caption[Relación de la frecuencia angular entre la función $sin(t)$ con el circulo unitario.] {\textit{ Fuente: Elaboración propia}  }
...
Los armónicos son funciones sinusoidales que forman parte fundamental de la teoría de las series de Fourier, utilizados en la descomposición de señales.

\subsubsection{Definición de Armónicos}
Los armónicos se definen como múltiplos enteros de una frecuencia fundamental. Son componentes básicos en el análisis de series de Fourier.

\subsubsection{Importancia de los Armónicos en Series de Fourier}
En las series de Fourier, los armónicos permiten expresar funciones periódicas como una combinación de senos y cosenos de diferentes frecuencias.
...
La ortogonalidad entre funciones significa que su producto interno es cero. Esto permite la separación de componentes en una descomposición.

\subsubsection{Ejemplos de Funciones Ortogonales}
Ejemplos de funciones ortogonales incluyen los senos y cosenos en series de Fourier, y los polinomios de Legendre y Chebyshev.

\subsection{Originalidad del Seno y Coseno}
El seno y el coseno son fundamentales en el análisis de Fourier debido a sus propiedades de periodicidad y ortogonalidad.

\subsubsection{Propiedades del Seno y el Coseno}
El seno y el coseno tienen propiedades importantes, como su periodicidad y simetría, que los hacen útiles en series de Fourier.

\subsubsection{Uso del Seno y el Coseno en Series de Fourier}
En series de Fourier, el seno y el coseno se usan para representar funciones periódicas a través de la descomposición en frecuencias.
...
Una función suave es infinitamente diferenciable en su dominio, lo que facilita su análisis en series.

\subsubsection{Definición de Funciones Suaves por Partes}
Una función suave por partes es diferenciable en segmentos específicos, con algunas discontinuidades en sus derivadas.

\subsubsection{Aplicaciones en Series de Fourier}
Estas funciones son útiles en el análisis de Fourier debido a sus propiedades de continuidad y diferenciabilidad.
